...
// PERUBAHAN 1: Menggunakan class Client secara langsung
use Gemini\Client;
...
class KomregController extends Controller
{
     ...
     */
    // PERUBAHAN 2: Meminta Laravel untuk memberikan object Client Gemini secara langsung
    public function analyze(Request $request, Client $gemini)
    {
        $request->validate([
            'images.*' => 'required|image'
        ]);

        $prompt = "Anda adalah asisten data ekstraksi yang sangat akurat. Tugas Anda adalah menganalisa gambar screenshot dari sebuah aplikasi e-commerce dan mengekstrak nilai persentase komisi untuk setiap produk berdasarkan aturan ketat berikut, lalu mengembalikan hasilnya dalam format JSON array.

        ATURAN EKSTRAKSI:
        1.  Satu unit produk diidentifikasi oleh adanya ikon \"centang oranye\" di sisi paling kiri. Proses dari atas ke bawah.
        2.  Untuk setiap produk yang memiliki centang oranye, pastikan produk tersebut valid dengan cara menemukan teks harga (contoh: Rp21.500) yang berwarna oranye pekat di baris yang sama.
        3.  Jika produk valid, cari teks \"Komisi hingga [angka]%\" yang berada di dekat harga. Ekstrak HANYA ANGKA numeriknya saja.
        4.  Jika sebuah produk valid (memiliki centang oranye dan harga oranye) tetapi teks \"Komisi hingga [angka]%\" tidak ditemukan, maka nilai komisi untuk produk tersebut adalah 0.
        5.  Urutan angka dalam hasil akhir HARUS sesuai dengan urutan produk dari atas ke bawah di dalam gambar.

        FORMAT OUTPUT:
        Hanya berikan jawaban dalam format JSON array berisi angka (integer). Jangan menyertakan teks, penjelasan, atau format markdown apa pun selain JSON array itu sendiri.

        Contoh output yang diharapkan: [15, 5, 17, 12, 3]";

        $allCommissions = [];

        foreach ($request->file('images') as $image) {
            try {
                // PERUBAHAN 3: Memanggil AI dengan cara yang paling stabil
                $result = $gemini->gemini('gemini-pro-vision')
                    ->generateContent([
                        $prompt,
                        Content::blob(
